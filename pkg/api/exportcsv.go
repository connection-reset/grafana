...
func Align(series [][]Point) []Row {
	...
	for {
		...
		var row Row
		row.Time = time
		row.Values = make([]string, len(series))
...
func queryInfluxDB(query string, ds *m.DataSource) ([]Point, error) {
	...
	for i, point := range response.Results[0].Series[0].Values {
		...
		if point[1] == nil {
			...
			// i think there are bools somewhere too
			switch point[1].(type) {
			case json.Number:
				points[i].Value = point[1].(json.Number).String()
			case string:
				points[i].Value = point[1].(string)
			}
		}
	}
	return points, nil
